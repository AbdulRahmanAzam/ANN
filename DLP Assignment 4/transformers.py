...
def generate_token_dict(vocab):
    ...
    token_dict = {}
    # assign a unique integer to each token in the vocab
...
def prepocess_input_sequence(
    ...
    sequence "33", it is broken down into a list of digits,
    ['0', '3'] and assigned to a corresponding value in the token_dict.
    ...
    """
    out = []
    # map tokens and digits to their dictionary values
...
def scaled_dot_product_two_loop_single(
    ...
    dot product. We map the input query, key, and value to the output.
    ...
    out = None
    # compute scaled dot product attention using loops
    K, M = query.shape
    out = torch.zeros_like(query)
    for i in range(K):
        weights = torch.zeros(K, device=query.device, dtype=query.dtype)
        for j in range(K):
            weights[j] = torch.dot(query[i], key[j]) / math.sqrt(M)
...
def scaled_dot_product_two_loop_batch(
    ...
    out = None
    N, K, M = query.shape
    # batch version of scaled dot product attention using loops
    out = torch.zeros_like(query)
    for n in range(N):
        for i in range(K):
            weights = (query[n, i] @ key[n].t()) / math.sqrt(M)
            weights = torch.softmax(weights, dim=0)
            out[n, i] = weights @ value[n]
    return out
...
def scaled_dot_product_no_loop_batch(
    ...
    # vectorized scaled dot product attention with masking
    weights = torch.bmm(query, key.transpose(1, 2)) / math.sqrt(M)
    if mask is not None:
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        # initialize query, key, and value linear layers with uniform weight initialization
        self.q = nn.Linear(dim_in, dim_q)
        self.k = nn.Linear(dim_in, dim_q)
        self.v = nn.Linear(dim_in, dim_v)
        
        for layer in [self.q, self.k, self.v]:
            c = math.sqrt(6.0 / (layer.in_features + layer.out_features))
    ...
    def forward(
        ...
        y = None
        # transform inputs and compute scaled dot product attention
        q_trans = self.q(query)
        k_trans = self.k(key)
        v_trans = self.v(value)
        
        y, self.weights_softmax = scaled_dot_product_no_loop_batch(q_trans, k_trans, v_trans, mask)
        return y
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        # set up the parallel attention heads
        self.heads = nn.ModuleList([SelfAttention(dim_in, dim_out, dim_out) for _ in range(num_heads)])
        
        # layer to combine all head outputs
        self.output_linear = nn.Linear(num_heads * dim_out, dim_in)
        
        # custom weight init as per instructions
        init_range = math.sqrt(6.0 / (self.output_linear.in_features + self.output_linear.out_features))
        nn.init.uniform_(self.output_linear.weight, -init_range, init_range)
    ...
    def forward(
        ...
        """
        y = None
        # collect head outputs and project combined result
        # get outputs from all heads
        results = []
        for h in self.heads:
            results.append(h(query, key, value, mask))
            
        # glue them together and project back
        combined = torch.cat(results, dim=-1)
        y = self.output_linear(combined)
        return y
...
class LayerNormalization(nn.Module):
    def __init__(self, emb_dim: int, epsilon: float = 1e-10):
        ...
        # initialize learnable scale (gamma) and shift (beta) parameters
    ...
    def forward(self, x: Tensor):
        ...
        """
        y = None
        # normalize input across the last dimension
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        # initialize two linear layers for the feed forward block
    ...
    def forward(self, x):
        ...
        """
        y = None
        # forward pass through linear layers and ReLU activation
        y = self.mlp2(F.relu(self.mlp1(x)))
        return y
...
class EncoderBlock(nn.Module):
    def __init__(
        ...
        As shown in Figure 1 of the "Attention is All You Need" paper
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        # initialize attention, normalization, feed-forward, and dropout layers
    ...
    def forward(self, x):
        ...
        """
        y = None
        # encoder block forward pass with residual connections
        # self-attention part
        attn_out = self.MultiHeadBlock(x, x, x)
        res1 = self.dropout(self.norm1(attn_out + x))
        
        # feed forward part
        ff_out = self.feed_forward(res1)
        y = self.dropout(self.norm2(ff_out + res1))
        return y
...
def get_subsequent_mask(seq):
    ...
    """
    mask = None
    # construct look-ahead mask for the decoder
    N, K = seq.shape
...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        self.dropout = None
        self.feed_forward = None
        # initialize self-attention, cross-attention, feed-forward, and normalization layers
    ...
    def forward(
        ...
        """
        y = None
        # decoder block forward pass with self and cross attention
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    """
    y = None
    # define simple linear positional encoding
    # generate values from 0 to (K-1)/K
    vals = torch.arange(K).float() / K
    # reshape and tile to match (1, K, M)
    y = vals.view(1, K, 1).repeat(1, 1, M)
    return y
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    encoding = torch.zeros(K, M)
    
    # indices along embedding dimension
    j = torch.arange(M)
    # i and a as defined in the notebook formula
    i = j // 2
    a = (2 * i // M).float()
    divisor = 10000 ** a
    ...
    encoding[:, 0::2] = torch.sin(pos / divisor[0::2])
    encoding[:, 1::2] = torch.cos(pos / divisor[1::2])
    
    # add batch dimension
    y = encoding.unsqueeze(0)
    return y
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        # step 1: run the encoder on the question
        e_output = self.encoder(q_emb_inp)
        
        # step 2: get the look-ahead mask for the answer sequence
        lookahead_mask = get_subsequent_mask(ans_b[:, :-1])
        
        # step 3: run the decoder
        dec_out = self.decoder(a_emb_inp, e_output, lookahead_mask)
        
        # reshape for loss: (N, O, V) -> (N*O, V)
        N, O, V = dec_out.shape
